...
const continente = document.querySelector('.continenteTexto');
const capital = document.querySelector('.capitalTexto');
const lingua = document.querySelector('.linguaTexto');
const ponto = document.querySelector('.ponto');
const pontoTexto = document.querySelector('.pontos');
const resultado = document.querySelector('.resultado');
const populacaoCard = document.querySelector('.populacao');
const dinheiroCard = document.querySelector('.dinheiro');
const continenteCard = document.querySelector('.continente');
const capitalCard = document.querySelector('.capital');
const linguaCard = document.querySelector('.lingua');
...
const countryNames = Object.keys(countriesFlags);
let vidas = 5;


function saveHighScore(score) {
    let highScoreGuess = localStorage.getItem('highScoreGuess');

    if (highScoreGuess === null || score > highScoreGuess) {
...
function loadHighScore() {
    let highScoreGuess = localStorage.getItem('highScoreGuess');
    if (highScoreGuess === null) {
        highScoreGuess = 0; // Se não houver pontuação máxima salva, inicie com 0
    }

    document.getElementById('highScoreDisplay').innerText = highScoreGuess;
}
...
fetchCountryDataWithRetry(countryNames)
    .then(data => {
        ...
        if (data && data.continents && Array.isArray(data.continents)) {
            const continentes = data.continents.join(', ');
            continente.textContent = continentes;
        } else {
            console.error('Estrutura de continentes inválida:', data);
        }
        pontoTexto.textContent = pontos;
        const firstLanguageName = data.languages[Object.keys(data.languages)[0]];
        lingua.textContent = firstLanguageName;
        ...
        // Aqui você pode realizar outras operações com os dados, se necessário
    })
    .catch(error => {
...
const vida5 = document.querySelector('.vida5');
const vida4 = document.querySelector('.vida4');
...
const vidasCard = document.querySelector('.vidas');
    ...
    if (inputBox.value == selectedCountry) {
        ...
        switch (vidas) {
            ...
            default:
                ...
                pontuacao.textContent = pontos + " países corretos";
